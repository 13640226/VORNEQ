...
class MarketplacePrimaryNavigationTests(TestCase):
    ...
    def test_library_index_redirect_preserves_query_string(self):
        with translation.override("en"):
            library_url = reverse("legacy_library_index")
            marketplace_url = reverse("marketplace:index")
            response = self.client.get(library_url, {"q": "shared topic"})

        self.assertEqual(response.status_code, 301)
        self.assertEqual(response.url, f"{marketplace_url}?q=shared+topic")
